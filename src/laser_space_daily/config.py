...
class DiscoverySettings(BaseModel):
    ...
    daily_search_budget: int = Field(default=20, ge=0, le=20)
    daily_elastic_budget: int = Field(default=5, ge=0, le=5)
    ...
    def enforce_mode_budget(self) -> "DiscoverySettings":
        ...
        if self.daily_search_budget + self.daily_elastic_budget > 25:
            raise ValueError("daily total search budget must not exceed 25")
        return self
